...
public class ProfileController {
  ...
  @PutMapping("/me")
  public ProfileDto updateMyProfile(@Valid @RequestBody UpsertProfileRequest request) {
    UUID userId = SecurityUtil.currentUserId().orElseThrow(() -> new BadRequestException("Unauthorized"));
    return profileService.upsertProfile(userId, request);
  }

  @GetMapping("/me/posts")
  public PagedResponse<PostWithProfileDto> getMyPosts(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "20") int limit) {
    UUID userId = SecurityUtil.currentUserId().orElseThrow(() -> new BadRequestException("Unauthorized"));
    return postService.getPostsByUser(userId, page, limit);
  }

  @GetMapping("/me/liked-posts")
  public PagedResponse<PostWithProfileDto> getMyLikedPosts(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "20") int limit) {
    UUID userId = SecurityUtil.currentUserId().orElseThrow(() -> new BadRequestException("Unauthorized"));
